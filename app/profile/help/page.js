...
function FAQItem({ q, a }) {
  ...
  return (
    <div className="border-b border-[#c2c6d8]/30 last:border-0">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-start justify-between gap-3 px-4 py-4 text-left transition hover:bg-[#f3f3f8]"
      >
        <span className="text-sm font-semibold text-[#424656]">{q}</span>
        <ChevronDown className={`h-4 w-4 shrink-0 text-[#727687] transition-transform duration-200 mt-0.5 ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
        <p className="px-4 pb-4 text-sm leading-relaxed text-[#727687]">{a}</p>
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-20 glass border-b border-white/40 px-4 py-3">
        <div className="mx-auto flex max-w-5xl items-center gap-3">
          <button onClick={() => router.back()} className="flex h-10 w-10 items-center justify-center rounded-full border border-[#c2c6d8]/30 bg-white/80 text-[#424656] shadow-sm transition hover:border-primary/40 hover:text-primary active:scale-95">
            <ArrowLeft className="h-4 w-4" />
          </button>
          <div>
            <p className="text-xs text-[#727687]">Support</p>
            <h1 className="text-sm font-black text-[#1a1c1f]">Help & FAQ</h1>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-lg px-4 pb-28 pt-6 md:pb-10 space-y-4">
        ...
        <div className="rounded-2xl bg-white shadow-card overflow-hidden animate-slide-up">
          <p className="px-4 pb-1 pt-4 text-[10px] font-black uppercase tracking-widest text-[#727687]">Frequently Asked Questions</p>
          {FAQS.map((faq) => <FAQItem key={faq.q} q={faq.q} a={faq.a} />)}
        </div>

        {/* Contact */}
        <div className="rounded-2xl bg-white p-4 shadow-card animate-slide-up delay-75 space-y-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#727687]">Still need help?</p>
          <a href="tel:+919969272885"
            className="flex items-center gap-3 rounded-xl border border-[#c2c6d8]/30 px-3 py-3 transition hover:border-primary/30 hover:bg-primary/5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
              <Phone className="h-4 w-4" />
            </div>
            <div>
              <p className="text-sm font-bold text-[#424656]">Call Us</p>
              <p className="text-xs text-[#727687]">+91 99692 72885 · 24 / 7 support</p>
            </div>
          </a>
          <a href="mailto:support@garagedekho.com"
            ...
            <div>
              <p className="text-sm font-bold text-[#424656]">Email Support</p>
              <p className="text-xs text-[#727687]">support@garagedekho.com</p>
            </div>
          </a>
        </div>
